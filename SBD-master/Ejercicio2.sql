...
WHERE A.nom_alumno = 'SANCHEZ'
    AND C.nom_carrera = 'CONTADOR PUBLICO'
    AND E.nota_examen < C.nota_aprob_examen_final;
/*Cantidad de materias no aprobadas por el alumno 'SANCHEZ' en la carrera 'CONTADOR PUBLICO's*/
SELECT COUNT(*) - (
        SELECT COUNT(*)
        FROM EXAMENES e,
            MATERIAS m
        WHERE e.cod_carrera = m.cod_carrera
            AND e.cod_materia = m.cod_materia
            AND e.nota_examen >= m.nota_aprob_examen_final
            AND e.nro_alumno = (
                SELECT nro_alumno
                FROM ALUMNOS
                WHERE nom_alumno = 'SANCHEZ'
            )
            AND m.cod_carrera = (
                SELECT cod_carrera
                FROM CARRERAS
                WHERE nom_carrera = 'CONTADOR PUBLICO'
            )
    ) AS 'Diferencia'
FROM MATERIAS m
WHERE m.cod_carrera = (
