...
(
 ...
 anios_regularidad	tinyint			not null,
...
(
 ...
 anio_materia		smallint		not null,
...
(
 cod_carrera		smallint		not null,
 cod_materia		char(4)			not null,
 anio_cursado		smallint		not null,
 nro_alumno			integer			not null,
 regular			char(1)			not null,
 primary key (cod_carrera, cod_materia, anio_cursado, nro_alumno),
...
(
 ...
 foreign key (cod_carrera, cod_materia, anio_cursado, nro_alumno) references cursados
)
go
...
/* 2. Programar una funci�n que devuelva el a�o de vencimiento de la regularidad de una materia para un alumno determinado. 
   Esto se calcula sumando la cantidad de a�os que dura la regularidad (a�os_regularidad en tabla carreras) al a�o de cursado */

create function anio_vencimiento (@cod_carrera smallint, 
								  @cod_materia char(4), 
								  @nro_alumno integer)
returns smallint
as
begin
	declare @anio_cursado smallint
	declare @anios_regularidad smallint
	declare @resultado smallint

	set @anio_cursado = (select anio_cursado 
						 from cursados 
						 where cod_carrera = @cod_carrera 
						 and cod_materia = @cod_materia 
						 and nro_alumno = @nro_alumno)

	set @anios_regularidad = (select anios_regularidad 
							 from carreras 
							 where cod_carrera = @cod_carrera)

	set @resultado = @anio_cursado + @anios_regularidad
...
/*    Un alumno puede rendir una materia si:
   1. Est� regular en la misma, lo que implica:
      a. Tiene un registro en cursado con regular = 'S'
      b. Para ese registro tiene menos aplazos que la cantidad m�xima definida para la carrera
      c. Esa regularidad no est� vencida (el a�o de cursado + a�os de regularidad definidos para la carrera es mayor al a�o actual)
   2. La materia no est� aprobada */
